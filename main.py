# Integrated mapping through Mongo DB, Pyautogui and fetching number from Mongodb
import os
import uuid
import datetime
import openai
...
if __name__ == "__main__":
    # Making a call
    document_id = call.make_call()

    # Check for transcription
    query = speech_to_text.transcribe_stream()

    # If transcription is received
    if query:
        chat_with_user()
    
    # Update status to 'processed' after the entire script is done
    if document_id:
        call.update_call_status(document_id, 'processed')
