...
class Transcriber:
    ...
    async def sender(self, ws, timeout=0.2):
    ...
    async def run(self, key):
        ...
        self.stream = p.open(format=FORMAT, channels=1, rate=16000, input_device_index=2, input=True, stream_callback=self.mic_callback)
